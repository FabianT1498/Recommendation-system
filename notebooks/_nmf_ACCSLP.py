...
def _beta_divergence(S, X, Z, U, H, W, V, beta, alpha, loss_function="kullback-leibler"):
    ...
    if loss_function == 1:
        ...
        # computes S * np.sum((WH/sum_WH) * log(UH /(WH/sum_WH))) only where X is nonzero
...
def _multiplicative_update_u(
    ...
    # Denominator
    if beta_loss == 1:
        if H_sum is None and V_sum is None:
...
def _multiplicative_update_h(
    ...
    # Denominator
    if beta_loss == 1:
        if U_sum is None and W_sum is None:
...
def _fit_multiplicative_update(
    ...
    error_at_init = _beta_divergence(S, X, Z, U, H, W, V, beta_loss, square_root=True)
...
class NMF_ACCSLP(_BaseNMF):
    ...
    init : {'random', 'nndsvd', 'nndsvda', 'nndsvdar', 'custom'}, default=nndsvda
        Method used to initialize the procedure.
        Valid options:
    ...
    def _check_params(self, X):
        ...
        # initialize method
        if self.init != "nndsvd":
            # This class only uses 'nndsvd' for matrix initialization
            raise ValueError(
                f"This class only uses 'nndsvd' for matrix initializacion")
    ...
    def fit_transform(self, S, Z, X, y=None, U=None, H=None):
        ...
        with config_context(assume_finite=True):
            U, H, n_iter = self._fit_transform(S, Z, X, U=U, H=H)
    ...
    def _fit_transform(self, S, Z, X, y=None, U=None, H=None, update_H=True):
        """Learn a NMF model for the data X and returns the transformed data.
        ...
        # initialize or check W and H
        U, H = self._check_w_h(S, U=None, H=None, update_H=update_H)
        W, R = self._check_w_h(X,  U=None, H=None, update_H=update_H)
        L, V = self._check_w_h(Z,  U=None, H=None, update_H=update_H)
